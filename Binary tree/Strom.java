...
public class Strom {
    ...
    public int sectiVsechny() {
        ...
        sum += sectiPotomky(koren);
        return sum;
    }

    private int sectiPotomky(Uzel u) {
        int sum = 0;
        if (u.getLevy() != null) {
            sum += u.getLevy().getHodnota() + sectiPotomky(u.getLevy());
        }
        if (u.getPravy() != null) {
            sum += u.getPravy().getHodnota() + sectiPotomky(u.getPravy());
        }
        return sum;
    }
    ...
    public String uzlyVhloubce(int depthv) {
        ...
        int depth = depthv;
        int tmpdepth = 1;
        String s = hloubka(koren, tmpdepth, depth);
        return s;
    }

    private String hloubka(Uzel u, int tmpdepth, int depth) {
        String s = "";
        if (tmpdepth == depth) {
            s = u.getHodnota() + " ";
        } else {
            if (u.getLevy() != null) {
                s += hloubka(u.getLevy(), tmpdepth+1, depth);
            }
            if (u.getPravy() != null) {
                s += hloubka(u.getPravy(), tmpdepth+1, depth);
            }
        }
        return s;
    }
